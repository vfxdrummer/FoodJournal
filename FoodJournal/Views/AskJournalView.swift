...
struct AskJournalView: View {
    ...
    private var confirmedVisits: [Visit]
    ...
    private func ask() async {
        ...
        do {
            let result = try await service.ask(question: question, visits: confirmedVisits)
            answer = result.answer
            // Resolve persistent IDs back to Visit objects
            referencedVisits = result.referencedVisitIDs.compactMap { id in
                confirmedVisits.first { $0.persistentModelID == id }
            }
        } catch {
